...
3. **How do networking and storage work in Docker?**  
   ...
   ווליומס- אמצעי האחסון מנוהלים על ידי רכיב הדמון, בוולימס הנתונים מאוחסנים בפייל סיסטם של המכונה המארחת של הקונטיינר (ההוסט), וכדי שהקונטיינר יוכל לעשות בו שימוש עליו לטעון לתוכו את הווליום הרלוונטי.
   בינד מונטס- בבינד מונטס נוצר קישור בין הקונטיינר לבין ניתוב מסויים במערכת הקבצים של המכונה המארחת, כאשר מבצעים בינד מאונטס, לקונטטיינר יש את היכולת לגשת לכל הקבצים והספריות שמאוחסנים תחת הניתוב המצוות לו.
   יש לעשות שימוש באחסון פרסיסטנטי כאשר אנו רוצים לשמור נתונים מסויימים גם לאחר שהקונטיינרים שעושים שימוש בהם נמחקים.

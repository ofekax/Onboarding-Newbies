...
3. **Storage & Fault Tolerance:**  Explain how HDFS divides files into blocks, uses replication (default factor three), and how it detects and recovers from node failures.
   ...
   כאשר datanode סוים הופך ללא זמין, יהיה עדיין ניתן לגשת לנתוני הקובץ.
...
מה סוגי המשאבים שניתן לנהל בHDFS (QUATA)?
הNAME QUATA:
מאפשר להגביל את כמות התיקיות והקבצים שתחת ניתוב מסוים במערכת הקבצים.
הSPACE QUATA:
מאפשר להגביל את מספר הביטים המקסימלי שיכול להיות מוקצה עבור קבצים שנמצאים תחת ניתוב תיקייה מסוימת במערכת הקבצים.

איך סנאפשוט עוזר לשחזר מידע והאם הוא נספר בקואטה ?
הוא עוזר לשחזר מידע בכך שהוא שומר תמונת מצב של מידע במערכת בנקודות זמן מסוימות ומקל בכך על שחזורם.
הסנאפשוט נספר בקואטה.

לחקור על דאטה לוקאליטי.
דאטה לוקאליטי זוהי דרך שבה הנתונים מאוחסנים פיזית קרוב ליחידת העיבוד שתצרוך אותם.

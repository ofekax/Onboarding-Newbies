...
למעשה השימוש בסרוויס חושף את הפודים בקלסטר בצורה קבועה, מה שמאפשר לקלסטרלתפקד באופן יציב גם כאשר חל שינוי בפודים (מה שקורה מספר רב של פעמים מכיוון שפודים הם רכיבים זמניים אשר עלולים להיווצר ולהימחק פעמים רבות).
   ...
   -הnamspaces- הפרדה בין סביבות שונות בתוך הקלסטר, בקוברנטיס עבור כל namespace אפשר לנהל הרשאות ולהקצות משאבים.
   באמצעות הגדרת role אפשר להגדיר הרשאות בתוך namespace ספציפי ובאמצעות הגדרת RoleBinding אפשר לתת  למשתמש או קבוצה מסוימת של משתמשים ROLE בתוך הnamespace (כלומר אפשר לתת למשתמש\ים הרלוונטים הרשאות בתוך namespace מסוים).
   ...
   ה- limit ranges: עבור כל פוד אפשר להגדיר טווח של משאבים לניצול באמצעות הגדרת ערכי הrequests - הכמות המינימלית של המשאבים שהפוד דורש כדי לעלות והlimits - הכמות המקסימלית של המשאבים שבהם הפוד יכול לעשות שימוש.
 -  ה-jobs : אובייקטים בקוברנטיס שיוצרים פודים (אחד לפחות) שנועדו לבצע משימה מסוימת ולאחר ביצוע המשימה הם נועדו להסתיים.
  - ה cronjobs : אובייקטים בקוברנטיס שיוצרים jobs לפי לוח זמנים קבוע (למשל כל יום, כל שבוע וכו...)
   -ה-DaemonSet: אובייקט בקוברנטיס שמשכפל פודים זהים על פני נודים שונים בקלסטר.
...
ניתן לחשפוף את סוג סרוויס זה באינטרנט הציבורי באמצעות השימוש ב: Ingress- אובייקט בקוברנטיס אשר מייצג API שמנהל גישה חיצונית לסרוויסים בקלסטר באמצעות הפרוטוקול HTTP/HTTPS.
הIngress מאפשר  לאכוף כללים שמנחים לניתוב תעבורת הרשת בקלסטר.
...
עבור כל בקשה שמתקבלת בסרוייס שבה יש התאמה בין כתובת הIP של הנוד לבין הפורט הממופה לו, הסרוויס ינתב את תעבורת הרשת לפוד הרלוונטי שבתוך הנוד בקלסטר,.
תקשורת בין הפודים:
לכל פוד בקלסטר מוצאת כתובת IP פנימית יחודית אשר מאפשרת לפודים לתקשר זה עם זה בתוך הקלסטר.
פודים שתחת אותו הנוד יכולים לתקשר אחד עם השני ישירות באמצעות הlocalhost דרך השימוש בכתובת הIP הפנימית שהוקצתה להם . (מכיוון שהתקשורת מתבצעת בשכבת הרשת ולא בשכבת התעבורה).
פודים שתחת אותו הקלסטר אך לא תחת אותו הנוד, קוברנטיס עושה שימוש בפתרונות טכנולוגיים אשר יוצרים שכבת רשת וירטואלית על פני כל הקלאסטר, בצורה כזו שהם יוכלו לתקשר באופן ישיר אחד עם השני באמצעות השימוש בכתובת הIP הפנימית שלהם.

5. What is Helm, and why is it used?  
   Explain charts, values.yaml, templating, and how Helm simplifies deployments.

הלם הוא כלי לניהול חבילות עבור קוברנטיס אשר מאפשר לנהל ולפרוס אפליקציות על פני קלסטרים של קוברנטיס בצורה עקבית ואחידה.
הCharts: הצארט הוא התבנית הבסיסית של כל פריסת אפליקציה בהלם.
הצארט מכיל בתוכו את כל הרכיבים הדרושים להגדרת ופריסת האפליקציה על גבי הקוברנטיס.
תבנית הצארט מורכבת מ:
-הקובץ chart.yaml - קובץ שמכיל מטא דאטה כל הצארט (כמו שמו, תיאורו, גרסתו וכו).
-הקובץ values.yaml - קובץ קונפיגורציה שבו ניתן להגדיר את כל הפרמטרים הנדרשים לצארט.
הערכים שמוגדרים בקובץ זה הינם ערכי ברירת המחדל של הצארט.
-התיקייה templates - תיקייה המכילה את כל קבצי הtemplate.
קבצי הטמפלט הינם קבצי yaml והם משמשים עבור יצירת הרכיבים (סרוויסים, פודים, קונפיגמאפס..) בקלסטר של קוברנטיס. 

הלם תומך בניהול ריפוסיטוריס אשר מאפשרים לאחסן לשתף ולפרוס צארטים בצורה פשוטה.


מה ההבדל בין דיפלוימנט לרפליקה סט ואיך המעבר בין רפליקה סט חדש לישן מתרחש.
כאשר מתצבעת שינוי גרסה בדיפלוימנט הוא מאפשר לעשות זאת באופן הדרגתי מבוקר, והוא יודע להתמודד עם כשלים במהלך שינוי הגרסה בשונה מרפליקה סט.
כאשר רוצים לעבור מרפליקה סט ישן לחדש יש להגדיר דיפלוימנט.

לחקור על preemptiony ותזמון רקווסט על חשבון לימית.
כאשר יש פוד שהמתזמן מנסה לתזמן אותו בשרת כלשהוא והוא לא מוצא לו מקום שעונה על דרישותיו ומאפשר את העלתו, הpreemption מחפש שרת שבו פודים בעלי עדיפות נמוכה יותר מהפוד.
במידה ונמצא שרת, הpreemption יסיר מהשרת את הפודים שהעדיפות שלהם נמוכה יותר על מנת שהמתזמן יוכל לתזמן את הפוד הרצוי באותו השרת.
תזמון request יתועדף על פני תזמון limit,  כלומר כאשר יש פוד שרוצה לעלות ואין שרת שבו הוא יכול לעלות מכיוון שיש פוד אחר שעושה בו שימוש במלא משאבים (כי ערך הלימית שלו גדול), הכמות של המשאבים שבה עושה שימוש הפוד ה"חזיר" תקטן כמה שצריך (כל עוד הוא עדיין עושה שימוש בכמות המשאבים המוגדרת ברקווסט) בשביל שהפוד שאותו המתזמן מנסה לתזמן יוכל לעלות באותו השרת.

למה להשתמש בנוד פורט ולא פשוט בינגרס?
הנוד פורט מאפשר גם לחשוף סרוויסים מתוך הקלסטר  למחוץ הקלסטר באמצעות פורטים ופרוטוקולים (HTTP/HTTPS) בשונה 
אינגרס מנהל גישה חיצונית לסרוויסים מתוך הקלסטר באמצעות השימוש בפרוטוקול HTTP/HTTPS.
לאחר שלקוח חיצוני רוצה לגשת לסרוויס בתוך הקלסטר, הוא למעשה שולח בקשת HTTP לינגרס, והוא מנתב את בקשתו לסרוויס הרלוונטי.
החסרון באינגרס הוא שהוא תלוי באינגרס קונטרולר, זה לא מספיק רק להגדיר אובייקט אינגרס בקוברנטיס, על מנת שהאובייקט יוכל לפעול כראוי יש להוריד אינגריס קונטרול מראש ולהגדיר אותו בקלסטר.
נוד פורט מאפשר לחשוף את הסרוויס מתוך לקלסטר למחוצה לא ללא תלות בפרוטוקול HTTP/HTTPS בשונה מאינגרס.

אילו אובייקטים נוספים נוצרים שאני יוצר ngres בopenshift? 
נוצרים routes.
